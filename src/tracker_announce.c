...
static char *build_http_request(url_t *url, tracker_announce_request_t *request)
{
  ...
  written += print_url_encoded_str(buff + written, sizeof(buff) - written, (const unsigned char *)(request->info_hash), 20);

  written += snprintf(buff + written, sizeof(buff) - written, "&peer_id=");
  written += print_url_encoded_str(buff + written, sizeof(buff) - written, (const unsigned char *)(request->peer_id), 20);
  ...
  char *ret = (char *)malloc(written + 1);
  if (ret) {
    strcpy(ret, buff);
  }
